...
# Gamma_Si_grosserosion_DiMES_30nm = Gamma_Si_grosserosion_DiMES
# Time_30nm = Time
# Stopping_criteria = 0.2
# Delta_implant_amorphous = 50e-9

runcell(0, '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_functions.py')
 
runcell('Reading geometry files', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_initial_run.py')

runcell('Initiallize all surfaces with concentrations equal to the Z at that surface', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_initial_run.py')

Stopping_criteria = 0.1
Delta_implant_amorphous = 20e-9

for run_no in [2,3,4,5,6,7,8]:#[1,2,3,4,5]:
    ...
    #     Delta_implant_amorphous = 40e-9    
    # elif run_no<6:
    #     Stopping_criteria = 0.2
    #     Delta_implant_amorphous = 10e-9
    # elif run_no<8:
    #     Stopping_criteria = 0.2
    #     Delta_implant_amorphous = 10e-9        
    # elif run_no<9:
    ...
    # if run_no < 4:
    #     Stopping_criteria = 0.1
    #     Delta_implant_amorphous = 25e-9
    # elif run_no<6:
    #     Stopping_criteria = 0.12
    #     Delta_implant_amorphous = 15e-9
    # elif run_no<8:
    #     Stopping_criteria = 0.22
    #     Delta_implant_amorphous = 5e-9        
    # elif run_no<9:
    #     Stopping_criteria = 0.05
    #     Delta_implant_amorphous = 3e-9    
    # else:
    #     Stopping_criteria = 0.12
    runcell('Reading position files of Carbon', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')

    runcell('Reading geometry files', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')

    runcell('Calculation of erosion and deposition fluxes for Carbon and Silicon for each GITR particle', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')

    runcell('The following arrays will keep track of entries to be made in the next GITR run.', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')

    #if run_no == 8:
    #    runcell('making particle sources', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')
        
    runcell('Estimating the total time evolution for the surface model', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')

    runcell('Appending time to all the surface characteristics', '/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/surface_model_subsequent_runs.py')
    
print("--------------------")
#print("Characteristic time in seconds")    
#surface_in_question = 42
...
# print("--------------------")
# print("Characteristic time in seconds")    
surface_in_question = 150
# 1/np.polyfit(np.log(Surface_time), Concentration[20][surface_in_question,:], 1)    

import matplotlib.pyplot as plt    

FileNameSurfaceConcentration='/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/input/surface_evolution_C_Si.nc'
...
plt.ylabel("Concentrations",fontsize=18,color='blue')
plt.xlabel("Time (s)",fontsize=18,color='blue')
#plt.xlim(0,7)
plt.rcParams["axes.edgecolor"] = "black"
plt.rcParams["axes.linewidth"] = 2
plt.title("Representative surface mesh element for Amorphous",fontsize=18)
...
FileNameHistory='/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/output_C_'+str(run_no)+'/positions.nc'
...
Angles_particles_C = np.arctan(np.sqrt(surface_vx_C**2+surface_vy_C**2)/surface_vz_C)*(180/np.pi)   # in degrees
...
FileNameHistory='/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/output_Si_'+str(run_no)+'/positions.nc'
...
Angles_particles_Si = np.arctan(np.sqrt(surface_vx_Si**2+surface_vy_Si**2)/surface_vz_Si)*(180/np.pi)    #  in degrees
...
for i in range(len(Energy_particles_C)):
    ...
    if surfacehit_C[i] != -1 and int(surfacehit_C[i]) in Surfaces:
        ...
        Y_CSiC_Si_Gamma_C_redep[surface_index] = Y_CSiC_Si_Gamma_C_redep[surface_index] + sr_object.Calculate_PhysicalSputteringParameters('C',Target_2,Energy_particles_C[i],Angles_particles_C[i])*Flux_C_local
        Y_CSiC_C_Gamma_C_redep[surface_index] = Y_CSiC_C_Gamma_C_redep[surface_index] + sr_object.Calculate_PhysicalSputteringParameters('C',Target_1,Energy_particles_C[i],Angles_particles_C[i])*Flux_C_local
        ...
        Y_CSien_Gamma_C_redep[surface_index] = Y_CSien_Gamma_C_redep[surface_index] + sr_object.Calculate_PhysicalSputteringParameters('C','Si',Energy_particles_C[i],Angles_particles_C[i])*Flux_C_local
...
makeParticleSource('C',particleSourceDict_C, "/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/input/gitrGeom.cfg", "/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/input/particleConf_C.nc")
if nP_Si_global>0:     
    makeParticleSource('Si',particleSourceDict_Si, "/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/input/gitrGeom.cfg", "/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/input/particleConf_Si.nc")
...
for surface_index in range(len(Surfaces)):
    ...
    if Total_net_flux > 0: # deposition regime
        ...
        if surface_index in Surfaces_SiC:
            ...
            Gamma_C_bulk[surface_index] = Total_net_flux  #(Gamma_C_net[surface_index]+Gamma_Si_net[surface_index]+Gamma_SiC_net[surface_index])
            Gamma_Si_bulk[surface_index] = 0.0
            Gamma_SiC_bulk[surface_index] = 0.0
            Gamma_Cenr_bulk[surface_index] = 0.0
            Gamma_Sienr_bulk[surface_index] = 0.0
...
Delta_t_surface_estimate_SiC = (Delta_implant_amorphous*n_atom_C*Stopping_criteria)/RHS_SiC
...
SiC_last_entry_SiC = np.reshape(Concentration[20][Surfaces_SiC,-1],(len(Surfaces_SiC),1))
SiC_new_entry_SiC = np.reshape(Concentration[20][Surfaces_SiC,-1],(len(Surfaces_SiC),1))
...
for t in range(1,int(Time_steps)):
    ...
    for surface_index in range(len(Surfaces)):
        ...
        if Total_net_flux > 0: # deposition regime
            ...
            if surface_index in Surfaces_SiC:    
                ...
                Gamma_C_bulk[surface_index] = Total_net_flux#(Gamma_C_net[surface_index]+Gamma_Si_net[surface_index]+Gamma_SiC_net[surface_index])
                Gamma_Si_bulk[surface_index] = 0.0
                Gamma_SiC_bulk[surface_index] = 0.0 # Total_net_flux            
                Gamma_Cenr_bulk[surface_index] = 0.0
                Gamma_Sienr_bulk[surface_index] = 0.0
               
    #print(t)
    new_entry_C = new_entry_C + Delta_Time*(Gamma_C_net - Gamma_C_bulk)/(Delta_implant_amorphous*n_atom_C)    
    new_entry_Si = new_entry_Si + Delta_Time*(Gamma_Si_net - Gamma_Si_bulk)/(Delta_implant_amorphous*n_atom_C)    
    new_entry_SiC = new_entry_SiC + Delta_Time*(Gamma_SiC_net - Gamma_SiC_bulk)/(Delta_implant_amorphous*n_atom_C)
    new_entry_Cenr = new_entry_Cenr + Delta_Time*(Gamma_Cenr_net - Gamma_Cenr_bulk)/(Delta_implant_amorphous*n_atom_C)
    new_entry_Sienr = new_entry_Sienr + Delta_Time*(Gamma_Sienr_net - Gamma_Sienr_bulk)/(Delta_implant_amorphous*n_atom_C)
    ...
    #     break
    if (np.abs(new_entry_SiC[Surfaces_SiC]-last_entry_SiC[Surfaces_SiC])>Stopping_criteria).any():
        print(Delta_t_Stopping," Delta_t_Stopping ", t)
        break    
        
        
        
    #if (np.abs(SiC_new_entry_C-SiC_last_entry_C)>Stopping_criteria).any() or (np.abs(SiC_new_entry_Si-SiC_last_entry_Si)>Stopping_criteria).any() or (np.abs(SiC_new_entry_SiC-SiC_last_entry_SiC)>Stopping_criteria).any():
    #    print(Delta_t_Stopping," Delta_t_Stopping ", t)
    #    break
...
ncFile = netCDF4.Dataset('/Users/de/Research/DIIIDsurface_pyGITR/examples/DIMES_7/input/surface_evolution_C_Si.nc', 'w', format='NETCDF4')
