...
for l in range(total_runs):
    ...
    Delta_tt = Surface_time[time_index] - Surface_time[time_index-1]
    print(Surface_time[time_index])
    
    Gamma_C_grosserosion_absolute[:,0]  = Gamma_C_grosserosion_absolute[:,0] + Delta_tt* Gamma_C_ero_global[:,0]
    Gamma_Si_grosserosion_absolute[:,0] = Gamma_Si_grosserosion_absolute[:,0] + Delta_tt* Gamma_Si_ero_global[:,0]
    
    Gamma_C_neterosion_absolute[:,0]    = Gamma_C_neterosion_absolute[:,0] + Delta_tt* (Gamma_C_ero_global - Gamma_C_redep)[:,0]
    Gamma_Si_neterosion_absolute[:,0]   = Gamma_Si_neterosion_absolute[:,0] + Delta_tt* (Gamma_Si_ero_global - Gamma_C_redep)[:,0]
...
plt.ylabel(r"Erosion $\times(10^{21} m^{-2}) $",fontsize=18,color='blue')
