...
Flux_H = 1.0e20
alpha_c = 0.28      # Carbon concentration in the background plasma
...
Flux_H_Background = np.zeros(len(Surfaces))
Flux_C_Background = np.zeros(len(Surfaces))
YHtoSiC_Flux_H_in = np.zeros(len(Surfaces))
YCtoSiC_Flux_C_in = np.zeros(len(Surfaces))
YHtoSi_Flux_H_in  = np.zeros(len(Surfaces))
YHtoC_Flux_H_in   = np.zeros(len(Surfaces))
YCtoSi_Flux_C_in  = np.zeros(len(Surfaces))
YCtoC_Flux_C_in   = np.zeros(len(Surfaces))

Numbers           = np.zeros(len(Surfaces))

sr_object = Sputtering_and_reflection()

for j in Surfaces:
    for idr,r in enumerate(R):        
        if(PointInTriangle (r, Z[idr], x1[j], z1[j], x2[j], z2[j], x3[j], z3[j])):
            Energy_ion_H = 2*T_i[idr]+3*1*T_e[idr]
            Energy_ion_C = 2*T_i[idr]+3*6*T_e[idr]  # check this
            
            
            Gamma_H = 0.61 * ni[idr] * np.sqrt(k_B*T_e[idr]*11600/M_H) * np.sin(1.5*np.pi/180) # ni(m^-3),Te(eV),1.5 degrees DiMES angle, 1eV =  11600 K, in m^-2 s^-1
            Gamma_C = 0.28 * Gamma_H  # in m^-2 s^-1
            
            Flux_H_Background[j] = Flux_H_Background[j] + Gamma_H
            Flux_C_Background[j] = Flux_C_Background[j] + Gamma_C
            
            YHtoSiC_Flux_H_in[j] = YHtoSiC_Flux_H_in[j] + sr_object.Calculate_PhysicalSputteringParameters('H','SiC',Energy_ion_H,60)*Gamma_H
            YCtoSiC_Flux_C_in[j] = YCtoSiC_Flux_C_in[j] + sr_object.Calculate_PhysicalSputteringParameters('C','SiC',Energy_ion_C,60)*Gamma_C
            YHtoSi_Flux_H_in[j]  = YHtoSi_Flux_H_in[j] +  sr_object.Calculate_PhysicalSputteringParameters('H','Si',Energy_ion_H,60)*Gamma_H
            YHtoC_Flux_H_in[j]   = YHtoC_Flux_H_in[j] +   sr_object.Calculate_PhysicalSputteringParameters('H','C',Energy_ion_H,60)*Gamma_H
            YCtoSi_Flux_C_in[j]  = YCtoSi_Flux_C_in[j] +  sr_object.Calculate_PhysicalSputteringParameters('C','Si',Energy_ion_C,60)*Gamma_C
            YCtoC_Flux_C_in[j]   = YCtoC_Flux_C_in[j] +   sr_object.Calculate_PhysicalSputteringParameters('C','C',Energy_ion_C,60)*Gamma_C
